...
class DNP3LLMPolicyEngine:
    ...
    def __init__(self, onos_ip="52.90.84.6", onos_port="8181", llm_model="llama3.1", ollama_url="http://localhost:11434", test_llm: bool = True):
    ...
    def real_llm_threat_analysis(self, intelligence: Dict) -> Dict:
        ...
        prompt = f"""You are a senior cybersecurity analyst specializing in industrial control systems (ICS/SCADA) and the DNP3 protocol.
Your job is to assess the threat and recommend a mitigation policy for the SDN controller.

You MUST base your reasoning ONLY on the information provided below.

ATTACK INTELLIGENCE (JSON):
{intelligence_json}

DNP3 FUNCTION CODE CONTEXT (use exactly this mapping):
- 13 (Cold Restart): CRITICAL - Forces complete system restart, causes downtime
- 18 (Stop Application): CRITICAL - Stops industrial processes, production halt
- 14 (Warm Restart): HIGH - Partial system restart, service disruption
- 21 (Disable Unsolicited): HIGH - Disables status reporting, blinds operators
- 130 (Unsolicited Response): MEDIUM - Status/data response, reconnaissance

THREAT ASSESSMENT RUBRIC (follow these rules step by step):
1. Function Code Severity
   - Critical: function_code in [13, 18]
   - High: function_code in [14, 21]
   - Medium: function_code == 130
   - Otherwise: Low
2. Attack Success
   - SUCCESS if success_indicators == True AND victim_response_count >= 3
   - PARTIAL if victim_response_count >= 1 but success_indicators is False/unknown
   - FAIL otherwise
3. Lateral Movement
   - cross_switch_attack == True means lateral movement (attacker_switch != victim_switch)
4. Detection Confidence
   - confidence close to 1.0 means highly trustworthy detection.

THREAT_LEVEL should be chosen with this guideline:
- CRITICAL: critical function code AND (successful attack OR lateral movement) AND confidence ≥ 0.8
- HIGH: high/critical function code with some success indicators OR lateral movement with medium confidence
- MEDIUM: suspicious / medium impact but limited success, or low confidence
- LOW: low impact, no success, low confidence

POLICY_RECOMMENDATION must be one of:
- IMMEDIATE_BLOCK  (for CRITICAL, immediate containment)
- SELECTIVE_BLOCK  (for HIGH, targeted blocking of DNP3 lateral movement)
- RATE_LIMIT       (for MEDIUM, suspicious but not fully confirmed)
- MONITOR          (for LOW, just enhanced monitoring)

CHAIN-OF-THOUGHT INSTRUCTIONS (think step by step):
1. First, carefully analyze the fields in ATTACK INTELLIGENCE:
   - attacker_ip, victim_ip, function_code, success_indicators, victim_response_count,
     attacker_switch, victim_switch, cross_switch_attack, confidence, severity.
2. Reason step by step about:
   - How dangerous the function_code is in ICS/SCADA terms.
   - Whether the attack appears successful.
   - Whether there is lateral movement (cross_switch_attack).
   - How trustworthy the detection is (confidence).
3. From this reasoning, derive:
   - A clear THREAT_LEVEL according to the rubric.
   - A POLICY_RECOMMENDATION that matches the THREAT_LEVEL.
4. Then produce the final answer in the exact template below.

IMPORTANT FORMAT REQUIREMENTS:
- Do NOT add any extra sections or headings.
- Do NOT invent new function codes or ICS concepts.
- Use 3–6 numbered lines in REASONING, each short and focused.
- THREAT_LEVEL, POLICY_RECOMMENDATION, CONFIDENCE_SCORE and EXPLANATION must each appear exactly once.

NOW RESPOND IN THIS EXACT FORMAT:

REASONING:
1) <first reasoning step, referencing specific fields from the JSON>
...
EXPLANATION: <1–2 sentences summarizing the threat and why this policy is appropriate."""
...
def main():
    ...
    parser.add_argument('--onos-ip', default='52.90.84.6', help='ONOS controller IP')
...
def handle_alert(alert: dict, onos_ip: str = "52.90.84.6", llm_model: str = "llama3.1", ollama_url: str = "http://localhost:11434"):
